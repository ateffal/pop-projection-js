TD_73_77 = [100000, 98471, 98360, 98281, 98220, 98167, 98120, 98076, 98035, 97997,
...
TD_90_92 = [100000, 99179, 99110, 99066, 99034, 99008, 98983, 98961, 98942, 98923,
...
TV_88_90 = [100000, 99352, 99294, 99261, 99236, 99214, 99194, 99177, 99161, 99145,
...
USA_2009 = [100000, 99363, 99322, 99295, 99274, 99259, 99244, 99230, 99218, 99207, 99198, 99189,
...
function add_mortality_table(Name, lx) {

    // check length of lx
    if (lx.length > 121) {
        console.log('The length of lx must be less than 121')
        return False;
    }

    //check that lx[i+1] <= lx[i]
    for (i = 0; i < n - 1; i++) {
        if (lx[i + 1] > lx[i]) {
            console.log('lx[i+1] must be <= lx[i]')
            return False;
        }
    }

    mortality_tables[name] = lx;
    console.log('Mortality table added successfully.')
    return True;
}
